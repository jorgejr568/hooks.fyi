...
function parseMultipart(
  ...
  textParts: Record<string, string | string[]>;
...
export async function parseRequest(
  ...
  if (hasBody) {
    ...
    if (!multipartParsed) {
      ...
      const slice = overflow
        ? rawBuf.subarray(0, opts.maxBodyPreviewBytes)
        : rawBuf;
      ...
      if (overflow) {
        ...
        const uploadBuf = truncatedAtFileCap
          ? rawBuf.subarray(0, opts.maxFileBytes)
          : rawBuf;
        rawBody = {
          bytes: new Uint8Array(
            uploadBuf.buffer,
            uploadBuf.byteOffset,
            uploadBuf.byteLength,
          ),
          contentType,
          truncatedAtFileCap,
        };
